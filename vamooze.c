 ...
 * This function appends characters from the 'src' string to the 'dest' string
 * up to a maximum of 'n' bytes, including the null terminator. It first
...
char *strn_cat(char *dest, char *src, int n)
{
        ...
        for (b = 0; src[b] != '\0' && b < n; b++) /* Copy up to n characters from src to dest */
	{
 ...
 *@n: The maximum number of characters to be copied, including the null terminator.
 * This function copies up to 'n' characters from the 'src' string to the 'dest'
 * string. It stops copying when 'n' characters have been copied, or when the end
 * of the 'src' string is reached, whichever comes first. If 'n' is greater than
